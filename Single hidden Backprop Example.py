...
inputlayerlength = 25345
hiddenlayer1length = 65
outputlayerlength = 4

inputlayer = [1 for a in range(inputlayerlength + 1)]
hiddenlayer1 = [1 for a in range(hiddenlayer1length + 1)]
outputlayer = [1 for a in range(outputlayerlength)]
...
def backProp(pattern,expectedoutput):
...
# a = feedForward([0,1,0,1,1,1,0,1,0])
# print(a)
...
# errors = []
print(" started ")
for iterC in range(2):
	inp = [random.randint(0,1) for a in range(inputlayerlength)]
	out = [random.randint(0,1) for a in range(outputlayerlength)]
	er = backProp(inp,out)
	print(er)
# 	errors.append(er)
# 	if iterC % 500 == 0:
